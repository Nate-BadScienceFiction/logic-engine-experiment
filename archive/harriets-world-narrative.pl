% Archived September 2026. An excerpt of the narrative layer of Harriet's World (a memory-plague
% epidemic), extracted from story prose by an LLM. It runs on the rewrite; the earlier engine returns
% no answers to its time-based queries. Known gaps: undeclared time points, one undefined name
% (loop_de_loopy), phases that never start, and an inconsistent origin for the outbreak.
